...
def main():
        ...
        if choice == 'EDA':
            ...
            uploaded_file1 = st.file_uploader("Upload .shp", type=".shp")
            uploaded_file2 = st.file_uploader("Upload .tif", type=".tif")

            baseline = "uploaded_file1"  # the path to baseline shapefile
            raster = "uploaded_file2"  # the path to raster file

            orig = pyosp.Orig_curv(baseline, raster, width=3000,
                                   line_stepsize=100, cross_stepsize=None)
            fig, ax = plt.subplots(figsize=(7, 3), dpi=300)
            orig.profile_plot(ax=ax, color="maroon")
            ax.set_xlabel("Length (m)")
            ax.set_ylabel("Depth (m LAT)")
            ax.title.set_text('NE8 Main Array')
            ax.grid()
        elif choice == 'About':
            st.subheader("About")
