...
def main():
        ...
        activities = ["EDA", "Plots", "Design Lines", "Geemaps", "Swath", "About"]
        ...
        if choice == 'EDA':
            ...
            # Customizable Plot
                ...
                type_of_plot = st.selectbox("Select Type of Plot", ["area", "bar", "line", "scatter", "histogram", "box", "kde"])

                if st.button("Generate Plot"):
                    st.success("Generating Customizable Plot of {}".format(type_of_plot))
            ...
            baseline = st.file_uploader("Upload .shp", accept_multiple_files=True)
            raster = st.file_uploader("Upload .tif")
            number1 = st.number_input('Enter your Width (buffer in metres): ', value = 1000)
            number2 = st.number_input('Enter your line stepsize (distance between points): ', value = 110)
            user_input1 = st.text_input("Figure Title is:")
            user_input2 = st.text_input("Figure X-Axis is:")
            user_input3 = st.text_input("Figure Y-Axis is:")

            orig = pyosp.Orig_curv(baseline, raster, width=number1,
                                   line_stepsize=number2, cross_stepsize=None)
            fig, ax = plt.subplots(figsize=(7, 3), dpi=300)
            orig.profile_plot(ax=ax, color="maroon")
            ax.set_xlabel(user_input2)
            ax.set_ylabel(user_input3)
            ax.title.set_text(user_input1)
            st.pyplot(fig)
